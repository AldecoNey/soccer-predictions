...
**Fecha:** 2026-09-19
**Estado:** Propuesta para aprobación del usuario (ver Sección P)

Este documento consolida la visión del producto (brief original del usuario) en una arquitectura concreta. Las decisiones de detalle están en `docs/adr/` (Architecture Decision Records) — este documento las resume y las conecta; los ADRs son la fuente de verdad cuando haya cualquier discrepancia.
...
- **Costo:** $0/mes en el arranque; reserva de hasta $19/mes (API-Football Pro) si el volumen lo exige, requiere aprobación del usuario antes de contratarse.

## F. Estrategia de datos

Ver `docs/data/schema.md` para el esquema completo (16 tablas núcleo: `competitions`, `seasons`, `teams`, `players`, `matches`, `match_stats`, `player_availability`, `news_signals`, `data_sources`, `model_versions`, `feature_snapshots`, `prediction_runs`, `predictions`, `bookmaker_snapshots`, `results`, `evaluation_metrics`).
...
| Categoría | MVP gratuito | MVP recomendado |
|---|---|---|
| Frontend (Vercel) | $0 | $0 |
| Backend (Render free) | $0 | $0 |
| Base de datos (Neon free) | $0 | $0 |
| Scheduling (GitHub Actions, repo público) | $0 | $0 (o +$4/mes si se necesita repo privado vía GitHub Pro) |
| Datos (API-Football free) | $0 | $0, con reserva de hasta $19/mes (Pro) si el volumen lo exige |
| Benchmark de odds (The Odds API free) | $0 | $0 |
| **Total** | **$0/mes** | **$0-23/mes**, dentro del límite de $30/mes |
